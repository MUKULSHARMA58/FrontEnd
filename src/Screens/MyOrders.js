...
function MyOrders(){
    ...
    return(
        ...
        {data.map((el,i)=>(
            ...
            <>
            <div className="card"  key={i} style={{width:'90%'}}>
                <div className="card-body">
                    <h5 className="card-title">Order Date : {moment(el.date).format('MMMM Do YYYY, h:mm:ss a')}</h5>
                    <h5 style={{backgroundColor:"greenyellow"}}className="card-title">Order Number : {el.ord_number}</h5>
                    <h5 className="card-title">Payment Mode : {el.payment_mode}</h5>
                    <h5  style={{color: el.status == "Pending" ? "orange" : "green"}}className="card-title">Order Number : {el.status}</h5>
                    ...
                    <table className="table table-dark">
                        ...
                        <a href="#" className="btn btn-success">Total Amount : {calculateSubtotal(el)} </a>
                </div>
                </div>
